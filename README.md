...
2. Open the SQL editor and run `supabase/schema.sql` from this repo. It creates the `teams`, `team_members`, `stations`, and `scores` tables, plus `station_rankings` (per-station placement, for display only) and `spirit_leaderboard` (total spirit points per team across all stations) views. If you already ran `schema.sql` on this project before the spirit points sections existed, run `supabase/migrate_spirit_sections.sql` once to add the new columns.
